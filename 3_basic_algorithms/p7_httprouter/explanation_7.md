...
insert partial_path. When we reach the end, we add the handler. It takes O(m)
in time and space with m being the length of splitted path.

lookup: To look up a route we go through the partial path of each splitted path and
see if children has the route. It takes O(m) in time and O(1) in space with m
...
O(m) time to each operation with m being the length of path_string and O(k) in
space with k being the length of partials in the list.
